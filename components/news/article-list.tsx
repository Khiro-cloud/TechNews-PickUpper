...
export function ArticleList({ groups }: ArticleListProps) {
  ...
  return (
    <section className="grid gap-6 md:grid-cols-2 xl:grid-cols-3 xl:auto-rows-fr">
      {groups.map((group) => (
        <section
          className="flex min-h-[560px] flex-col rounded-[2rem] border border-black/5 bg-white/80 p-6 shadow-card backdrop-blur xl:h-[78vh] xl:max-h-[960px]"
          key={group.sourceKey}
        >
          <div className="mb-5 flex items-end justify-between gap-4 border-b border-black/8 pb-4">
            <div>
              ...
              <h2 className="mt-1 text-2xl font-semibold tracking-tight text-ink">{group.sourceName}</h2>
            </div>
